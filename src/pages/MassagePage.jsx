...
function CornerStickyCTA({ visible, onAction, onClose, price = "360 000 сум" }) {
      ...
      <div className="max-w-[360px] w-[92vw] sm:w-[360px] rounded-2xl shadow-xl border border-ink/10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
        <div className="p-4">
          <p className="font-bebas text-[20px] leading-tight text-[#161A1D]">
            Запишитесь на пробный общий массаж <span className="text-scarlet">всего за {price}</span>
          </p>
...
export default function MassagePage() {
  ...
  const handleHideCornerCTA = useCallback(() => {
    setCtaVisible(false);
    ...
    <>
              ...
              >
                ...
                >
                  ...
                  <span className="block sm:inline">
                    В{" "}
                    <span className="font-xolo font-bold tracking-[-2px] sm:tracking-[-3px] md:tracking-[-5px]">
                ...
                <button type="button" onClick={() => setModalOpen(true)} className={`${ctaClasses} mt-3`}>
                ...
                <div className="flex flex-col">
                  <div className="bg-ink/5 rounded-xl p-5">
                    <h2 className="font-bebas text-[24px] md:text-[28px] leading-none">
                      {data.intro.title} <span className="text-scarlet">{data.studioName}</span>
                    </h2>
                    ...
                    <ul className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {data.intro.types.map((t, i) => (
                        <li key={i} className="flex items-start gap-2">
                          ...
                          <span className="font-helvCond text-[18px] md:text-[20px] text-[#161A1D]">{t}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      <Section className="bg-paper">
        <h2 className="font-bebas text-[28px] md:text-[36px] text-[#161A1D] mb-6 leading-tight">
          ЗАЧЕМ ПОСЕЩАТЬ МАССАЖ В СТУДИЯХ <span className="text-scarlet">R</span>E<span className="text-scarlet">F</span>ORMA?
        </h2>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {data.benefits.map((b, i) => (
              <article key={i} className="rounded-2xl overflow-hidden border border-ink/5">
                ...
                <div className="p-4">
                  <h3 className="font-bebas text-[20px] md:text-[22px] text-[#161A1D]">{b.title}</h3>
                  <p className="mt-1 font-helvCond text-[18px] md:text-[20px] text-[#161A1D]">{b.note}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <Section className="bg-paper" id="pricing">
        <div className="flex items-start justify-between gap-4 mb-4">
          <h2 className="font-bebas text-[28px] md:text-[36px] text-[#161A1D] leading-tight">ПРАЙС НА УСЛУГИ</h2>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-soft mb-6">
          <h3 className="font-bebas text-[22px] md:text-[26px] text-[#161A1D] mb-3">{data.pricing?.servicesTitle}</h3>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-soft mt-6">
          <h3 className="font-bебas text-[22px] md:text-[26px] text-[#161A1D] mb-3">{data.pricing?.hardwareTitle}</h3>
      ...
      {/* ================== Специалисты ================== */}
      <Section className="bg-paper">
        <h2 className="font-bebas text-[28px] md:text-[36px] text-[#161A1D] mb-6 leading-tight">{data.specialistsTitle}</h2>
          ...
          {data.specialists.map((s, i) => (
            <div key={i} className="bg-white rounded-2xl p-4 md:p-6 shadow-soft">
              <div className="grid grid-cols-[112px,1fr] gap-4 items-center">
                ...
                />
                <div>
                  <h3 className="font-bebas text-[22px] text-[#161A1D] leading-none">{s.name}</h3>
                  <p className="font-helvCond text-[18px] md:text-[20px] text-[#161A1D]/90">{s.role}</p>
                  <p className="mt-2 font-helvCond text-[16px] text-[#161A1D]/80">{s.description}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <Section id="studios" className="bg-paper">
        <h2 className="font-bebas text-[28px] md:text-[36px] text-[#161A1D] mb-6 leading-tight">{data.studiosTitle}</h2>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {data.studios.map((st, i) => (
            <article key={i} className="bg-white rounded-2xl overflow-hidden shadow-soft">
              ...
              <div className="p-5">
                <h3 className="font-bebas text-[22px] text-[#161A1D] leading-none">{st.name}</h3>
                <p className="mt-1 font-helvCond text-[18px] md:text-[20px] text-[#161A1D]">{st.address}</p>

                <div className="mt-3 flex flex-wrap items-center gap-4 text-[#161A1D]">
                  <span className="inline-flex items-center gap-1">
                    <Pin className="h-4 w-4 fill-current" />
                    <a href={st.mapLink} target="_blank" rel="noreferrer" className="underline decoration-ink/30 underline-offset-2 hover:decoration-ink">
                      Показать на карте
                    </a>
                  </span>
                  <span className="inline-flex items-center gap-1">
                ...
                <button type="button" onClick={() => setModalOpen(true)} className={`${ctaClasses} mt-4`}>
                  Записаться на массаж
                </button>
              </div>
            </article>
        ...
        <div className="relative z-10 flex items-center h-[360px] sm:h-[440px] md:h-[520px] lg:h-[580px]">
          <div className="px-7 md:px-12 lg:px-20 w-full">
            <div className="max-w-[820px]">
              <h3 className="font-bebas text-white leading-[0.9] tracking-tight text-[40px] sm:text-[56px] md:text-[72px] lg:text-[84px]">
                ЗАПИШИСЬ НА ПРОБНЫЙ СЕАНС
              </h3>
              <p className="mt-5 text-white/90 font-helvCond leading-relaxed text-[18px] sm:text-[20px] md:text-[22px] lg:text-[24px]">
                {data.trial.text}
              </p>
              <button type="button" onClick={() => setModalOpen(true)} className={`${ctaClasses} mt-6`}>
                {data.trial.cta}
              </button>
            </div>
          </div>
