...
import JsonLd from "../lib/JsonLd.jsx";
...
export default function Direction() {
  ...
  const studiosResolved = useMemo(
    () => studios.map((id) => allStudios.find((s) => s.id === id)).filter(Boolean),
    [studios]
  );
  ...
  return (
    <>
      {/* React 19 нативно поднимает <title>/<meta> в <head>; JSON-LD добавляем в head через JsonLd */}
        ...
        <Section className="bg-paper">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {benefitsBlock.items.map((b, i) => (
              <div key={b.title || i} className="flex gap-4 items-start bg-white rounded-2xl p-6 shadow-soft">
                <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-scarlet text-white mt-1" aria-hidden="true">
                  ✓
                </span>
                <div>
        ...
        {gallery?.length > 0 && (
          <Section className="bg-paper">
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {gallery.map((src, i) => (
                <div key={src || i} className="rounded-xl overflow-hidden relative">
                  <div className="pb-[66%]" />
                  <AvifPicture
                    src={src}
                    alt={`${title} фото ${i + 1}`}
