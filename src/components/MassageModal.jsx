...
export default function MassageModal({ open, onClose }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await sendLead({
        ...
        name,
        phone,
        studio,                // важен id филиала — сервер выберет чат
        extra: { studioTitle: stTitle },
      });
  ...
  const heroSrc = "/images/massage3.jpg";
                ...
                <div className="relative">
                  <select
                    ...
                    className="
                      ...
                      text-sm font-helvCond text-[#161A1D]/90 [appearance:none]
                    "
                  >
                    {modalStudios.map((s) => (
                  ...
                  <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#161A1D]/60" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M5.25 7.5l4.75 4.75L14.75 7.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                  </svg>
                </div>
              </div>
